...
#'
#' @description
#' * Mod `y_position`, `margin_top`, `step_increase`, `tip_length` to absolute position on y-axis.
...
#' @param test the name of the statistical test that is applied to the values of the 2 columns (e.g. `t.test`, `wilcox.test` etc.). If you implement a custom test make sure that it returns a list that has an entry called `p.value`.
...
#' @param y_position numeric vector with the y positions of the brackets, absolute value relative to y-axis.
...
#' @param margin_top numeric vector of the distance between brackets and maximum value of each plot (bar, box, violin...), absolute value relative to y-axis.
#' @param step_increase numeric vector with the increase for each additional comparison, absolute value relative to y-axis.
#' @param extend_line numeric that allows to shorten (negative values) or extend (positive value) the horizontal line between groups for each comparison; defaults 0.
#' @param tip_length numeric vector with tip length for each bracket, absolute value relative to y-axis.
#' @param parse If `TRUE`, the labels will be parsed into expressions and
#'   displayed as described in `?plotmath`.
#' @param manual Boolean flag that indicates that the parameters are provided with a data.frame. This option is necessary if one wants to plot different annotations per facet.
#' @param na.rm If `TRUE`(the default), silently removes missing values.If `FALSE`, removes missing values with a warning.
...
## geom_signif_wrapper
geom_signif_wrapper <- function(mapping = NULL,
                                data = NULL,
                                stat = "signif_wrapper",
                                position = "identity",
                                na.rm = TRUE,
  ...
  if (identical(stat, "signif_wrapper")) {
    ...
    if (!is.null(comparisons)) {
      ...
      result <- lapply(comparisons, function(comp) {
        ...
        if (scales$x$map(comp[1]) == data$group[1] | manual) {
          ...
          if (is.null(y_position)) {
            y_pos <- scales$y$range$range[2] + margin_top[i] + step_increase[i] * (i - 1)
          } else {
            y_pos <- y_position[i] + step_increase[i] * (i - 1)
          }
          data.frame(
            x = c(min(comp[1], comp[2]), min(comp[1], comp[2]), max(comp[1], comp[2])),
            xend = c(min(comp[1], comp[2]), max(comp[1], comp[2]), max(comp[1], comp[2])),
            y = c(y_pos - tip_length[(i - 1) * 2 + 1], y_pos, y_pos),
            yend = c(y_pos, y_pos, y_pos - tip_length[(i - 1) * 2 + 2]),
      ...
      if ((data$x[1] == min(complete_data$x) & data$group[1] == min(complete_data$group)) | manual) {
        ...
        data.frame(
          ...
          y = c(
            y_position - tip_length[seq_len(length(tip_length)) %% 2 == 1],
            y_position,
            y_position
          ),
          yend = c(
            y_position,
            y_position,
            y_position - tip_length[seq_len(length(tip_length)) %% 2 == 0]
          ),
